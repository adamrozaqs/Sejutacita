...
<script>
    ...
    $('#mySelect').select2({
        width: '100%',
        placeholder: "Choose Your Interest Topics",
        interest: true,
        multiple: true,
        tokenSeparators: [',']
    });
    
    var oldInterest = $('#interest > option[value={{ Request::old('interest') }}]');
    
    if (oldInterest.length > 0) {
        oldInterest.attr('selected', 'selected');
    }
    ...
    vals.forEach(function(e){
        console.log(e)
        if(!s2.find('option:contains(' + e.title + ')').length) 
        s2.append($(`<option>`).text(e.title));
        });
